...
if [ ! -e "./parser_w.exe" ]; then
    echo "Error: parser_w.exe not found!"
    exit 1
fi

if [ ! -x "./parser_w.exe" ]; then
    echo "Error: parser_w.exe not executable!"
    exit 1
fi
...
for test_file in $(find ./tests -type f -name "*.txt" | sort); do
    ...
    ./parser_w.exe ${taskNumber} < ${test_file} > ${output_file}
