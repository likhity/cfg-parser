...
if [ ! -e "./parser.exe" ]; then
    echo "Error: parser.exe not found!"
    exit 1
fi

if [ ! -x "./parser.exe" ]; then
    echo "Error: parser.exe not executable!"
    exit 1
fi
...
for test_file in $(find ./tests -type f -name "*.txt" | sort); do
    ...
    ./parser.exe ${taskNumber} < ${test_file} > ${output_file}
